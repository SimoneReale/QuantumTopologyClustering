...
from solvers import simple_simulated_annealing, p_median_kmedoids, quantum_annealing, sklearn_kmedoids
...
def test_pmedian(k, taxi_data_file="taxi_data/taxi_data_1.txt", filter_radius=10000, NUM_CLUSTERS_REDUCED = 1000, gamma = 1.0):
    Delta, n, df_5g, df_taxi, importance_values = create_reduced_dataset(NUM_CLUSTERS_REDUCED, filter_radius=filter_radius, is_importance_radius=True, taxi_data_file=taxi_data_file)
    selected_medoids_p_median = p_median_kmedoids(df_taxi, df_5g, k, importance_values, gamma=gamma) 
    return Delta, n, df_5g, df_taxi, importance_values, selected_medoids_p_median


def test_bqm_even_spread_simulated_annealing(k, taxi_data_file="taxi_data/taxi_data_1.txt", filter_radius=10000, NUM_CLUSTERS_REDUCED = 1000, c_p=10.0, c_s=1, lambda_=3.0, lagrange_multiplier=1.5, radius_model = 6000):
    Delta, n, df_5g, df_taxi, importance_values = create_reduced_dataset(NUM_CLUSTERS_REDUCED, filter_radius=filter_radius, is_importance_radius=True, taxi_data_file=taxi_data_file)
...
def test_bqm_even_spread_REAL_annealing(k, taxi_data_file="taxi_data/taxi_data_1.txt", filter_radius=10000, NUM_CLUSTERS_REDUCED = 1000, c_p=10.0, c_s=1, lambda_=3.0, lagrange_multiplier=1.5):
    Delta, n, df_5g, df_taxi, importance_values = create_reduced_dataset(NUM_CLUSTERS_REDUCED, filter_radius=filter_radius, is_importance_radius=True, taxi_data_file=taxi_data_file)
    bqm, _ = create_bqm_even_spread(
        n,
        k,
...
def test_bqm_kmedoid_native(k, taxi_data_file="taxi_data/taxi_data_1.txt", filter_radius=10000, NUM_CLUSTERS_REDUCED = 1000):
        Delta, n, df_5g, df_taxi, importance_values = create_reduced_dataset(NUM_CLUSTERS_REDUCED, filter_radius=filter_radius, is_importance_radius=True, taxi_data_file=taxi_data_file)
        alpha = 1/k
        beta = 1/n
...
def test_k_medoids_sklearn(k, taxi_data_file="taxi_data/taxi_data_1.txt", filter_radius=10000, NUM_CLUSTERS_REDUCED = 1000):
    Delta, n, df_5g, df_taxi, importance_values = create_reduced_dataset(NUM_CLUSTERS_REDUCED, filter_radius=filter_radius, is_importance_radius=True, taxi_data_file=taxi_data_file)
...
if __name__ == "__main__":
    ...
    for i in single_range:
        ...
        print(f"\nSTO GESTENDO {filename} ##################################################\n")
        if "k_medoids_native_bqm" in method:
            Delta, n, df_5g, df_taxi, importance_values, selected_medoids = test_bqm_kmedoid_native(k, filename, filter_radius=radius, NUM_CLUSTERS_REDUCED=NUM_CLUSTERS_REDUCED)
        elif "p_median_importance" in method:
            Delta, n, df_5g, df_taxi, importance_values, selected_medoids = test_pmedian(k, filename, filter_radius=radius, NUM_CLUSTERS_REDUCED=NUM_CLUSTERS_REDUCED, gamma = 0.2)
        elif "k_medoids_sklearn" in method:
            Delta, n, df_5g, df_taxi, importance_values, selected_medoids = test_k_medoids_sklearn(k, filename, filter_radius=radius, NUM_CLUSTERS_REDUCED=NUM_CLUSTERS_REDUCED)
        elif "bqm_even_spread_simulated_annealing" in method:
            Delta, n, df_5g, df_taxi, importance_values, selected_medoids = test_bqm_even_spread_simulated_annealing(k, filename, filter_radius=radius, NUM_CLUSTERS_REDUCED=NUM_CLUSTERS_REDUCED, c_p=10.0, c_s=1, lambda_=3.0, lagrange_multiplier=1.0, radius_model=4000)
        elif "bqm_even_spread_REAL_annealing" in method:
            Delta, n, df_5g, df_taxi, importance_values, selected_medoids = test_bqm_even_spread_REAL_annealing(k, filename, filter_radius=radius, NUM_CLUSTERS_REDUCED=NUM_CLUSTERS_REDUCED, c_p=10.0, c_s=1, lambda_=3.0, lagrange_multiplier=1.5)
        else:
            Delta, n, df_5g, df_taxi, importance_values, selected_medoids = test_pmedian(k, filename, filter_radius=radius, NUM_CLUSTERS_REDUCED=NUM_CLUSTERS_REDUCED, gamma = 0.2)
