...
def p_median_cqm(df_taxi, df_5g, k, importance_values, gamma=1.0):
    ...
    cqm.set_objective(quicksum(modified_Delta[i, j] * x[i, j] for i in range(n) for j in range(m)))

    # Constraint: Each demand point must be assigned to exactly one service location
    for j in range(m):
        cqm.add_constraint(quicksum(x[i, j] for i in range(n)) == 1, label=f'demand_assignment_{j}')
...
def p_median_bqm(df_taxi, df_5g, k, importance_values, gamma=1.0):
    ...
    # Constraint: Each demand point must be assigned to exactly one service location
    for j in range(m):
        bqm.add_linear_equality_constraint([(f'x_{i}_{j}', 1) for i in range(n)], constant=-1, lagrange_multiplier=1.0)

    # Constraint: A demand point can only be assigned to an active service location
    for i in range(n):
        for j in range(m):
            bqm.add_linear_inequality_constraint([(f'x_{i}_{j}', 1), (f'z_{i}', -1)], constant=0, lagrange_multiplier=1.0, label=f'assignment_active_{i}_{j}')

    # Constraint: Select exactly k medoids
    bqm.add_linear_equality_constraint([(f'z_{i}', 1) for i in range(n)], constant=-k, lagrange_multiplier=1.0)
...
if __name__ == "__main__":
    ...
    Delta, n, df_5g, df_taxi, importance_values = create_pechino_dataset(
        filter_radius=radius, is_importance_radius=True, taxi_data_file= f"taxi_data/taxi_data_{i}.txt"
    )
    ...
    print(f"cqm quadratic variables {cqm.num_quadratic_variables()} num of biases {cqm.num_biases()}")

    # Solve the problem using D-Wave's LeapHybridCQMSampler
